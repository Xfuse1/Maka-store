...
import { rateLimiter } from "@/lib/payment/rate-limiter"
...
export async function updateFraudRule(ruleId: string, updates: { is_active?: boolean; priority?: number }) {
  try {
    ...
    const { error } = await supabase.from("fraud_rules").update(updates).eq("id", ruleId)
...
export async function getPaymentStats() {
  try {
    ...
    const { data: statusCounts } = await supabase.from("payment_transactions").select("status")

    // Get risk level distribution
    const { data: riskCounts } = await supabase.from("payment_transactions").select("risk_level")

    // Get total amounts
    const { data: amounts } = await supabase.from("payment_transactions").select("amount, status")
